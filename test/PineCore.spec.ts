...
describe('LimitCore', function () {
  ...
  describe('Constructor', function () {
    it('should be depoyed', async () => {
      const contract = await LimitCore.new()
  ...
  describe('Deposit ETH', function () {
    it('reverts when sending ETH by mistake :: EOA', async () => {
      await assertRevert(
        web3.eth.sendTransaction({ from: user, to: limitCore.address, value: new BN(1) }),
        'LimitCore#receive: NO_SEND_ETH_PLEASE'
      )
    })

    it('reverts when sending ETH by mistake with data', async () => {
      await assertRevert(
        web3.eth.sendTransaction({ from: user, to: limitCore.address, value: new BN(1), data: '0x00112412' })
      )
    })
  })
  ...
  describe('Cancel ETH Order', function () {
    it('should cancel an ETH order', async () => {
      ...
      const exEtherSnap = await etherSnap(limitCore.address, 'LimitCore ETH')
      ...
      const depositTx = await limitCore.depositEth(
        encodedOrder,
        {
          value,
          gasPrice: 0,
      ...
      // Cancel order
      const cancelTx = await limitCore.cancelOrder(
        vaultFactory.address,         // Limit orders module
        ethAddress,                       // Sell ETH
        user,                             // Owner of the order
        witness,                      // witness
    ...
    it('reverts when cancel by hacker', async () => {
      ...
      // Cancel order
      await assertRevert(limitCore.cancelOrder(
        vaultFactory.address,         // Limit orders module
        ethAddress,                       // Sell ETH
        user,                             // Owner of the order
        witness,                      // witness
        data,                         // data
        fromOwner
      ), 'LimitCore#cancelOrder: INVALID_OWNER')
    })
  })
